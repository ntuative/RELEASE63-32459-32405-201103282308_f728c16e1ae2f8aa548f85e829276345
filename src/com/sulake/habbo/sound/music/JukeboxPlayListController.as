package com.sulake.habbo.sound.music
{
   ...
   public class JukeboxPlayListController extends GenericPlaylist implements IPlayListController
   {
      ...
      public function JukeboxPlayListController(param1:HabboSoundManagerFlash10, param2:HabboMusicController, param3:IEventDispatcher, param4:IEventDispatcher, param5:IConnection)
      {
         ...
         this.var_80 = param4;
         this.var_740 = [];
         this.var_740.push(new NowPlayingMessageEvent(this.onNowPlayingMessage));
         this.var_740.push(new JukeboxSongDisksMessageEvent(this.onJukeboxSongDisksMessage));
         for each(_loc6_ in this.var_740)
         {
            this._connection.addMessageEvent(_loc6_);
         }
         var_236.events.addEventListener(SongInfoReceivedEvent.TRAX_SONG_INFO_RECEIVED,this.onSongInfoReceivedEvent);
         this.var_80.addEventListener(RoomObjectSoundMachineStateUpdateEvent.const_635,this.onFurnitureStateOnEvent);
         this.var_80.addEventListener(RoomObjectSoundMachineStateUpdateEvent.const_766,this.onFurnitureStateOffEvent);
      }
      ...
      override public function dispose() : void
      {
         ...
         if(!_disposed)
         {
            ...
            if(false)
            {
               var_236.events.removeEventListener(SongInfoReceivedEvent.TRAX_SONG_INFO_RECEIVED,this.onSongInfoReceivedEvent);
            }
            if(this.var_80)
            {
               this.var_80.removeEventListener(RoomObjectSoundMachineStateUpdateEvent.const_635,this.onFurnitureStateOnEvent);
               this.var_80.removeEventListener(RoomObjectSoundMachineStateUpdateEvent.const_766,this.onFurnitureStateOffEvent);
               this.var_80 = null;
            }
            super.dispose();
            this.var_489 = null;
      ...
      {
         var_236.stop(HabboMusicPrioritiesEnum.const_412);
         this.var_471 = -1;
         var_566 = false;
      }
      ...
      private function onNowPlayingMessage(param1:IMessageEvent) : void
      {
         ...
         var_566 = _loc3_.currentSongId != -1;
         if(_loc3_.currentSongId >= 0)
         {
            var_236.playSong(_loc3_.currentSongId,HabboMusicPrioritiesEnum.const_412,_loc3_.syncCount / 1000);
            this.var_471 = _loc3_.currentSongId;
         }
         if(_loc3_.nextSongId >= 0)
         {
            var_236.addSongInfoRequest(_loc3_.nextSongId);
         }
         this.var_489.events.dispatchEvent(new NowPlayingEvent(NowPlayingEvent.const_834,_loc3_.currentSongId,_loc3_.currentPosition));
      }
      ...
      private function onJukeboxSongDisksMessage(param1:IMessageEvent) : void
      {
         ...
         while(_loc4_ < _loc3_.songDisks.length)
         {
            ...
            _loc7_ = var_236.getSongInfo(_loc5_) as SongDataEntry;
            if(_loc7_ == null)
            {
               _loc7_ = new SongDataEntry(_loc5_,-1,null,null,null);
               this.var_728.push(_loc5_);
               var_236.requestSongInfoWithoutSamples(_loc5_);
            }
         ...
         {
            _events.dispatchEvent(new PlayListUpdatedEvent(PlayListUpdatedEvent.const_574));
         }
      }
      ...
      private function onSongInfoReceivedEvent(param1:SongInfoReceivedEvent) : void
      {
         ...
         {
            _loc3_ = var_95[_loc2_] as SongDataEntry;
            if(_loc3_.id == param1.id)
            {
               _loc4_ = _loc3_.diskId;
               var_95[_loc2_] = var_236.getSongInfo(param1.id) as SongDataEntry;
               var_95[_loc2_].diskId = _loc4_;
               break;
            }
            _loc2_++;
         }
         while(this.var_728.indexOf(param1.id) != -1)
         {
            this.var_728.splice(this.var_728.indexOf(param1.id),1);
         }
         if(this.var_728.length == 0)
         {
            _events.dispatchEvent(new PlayListUpdatedEvent(PlayListUpdatedEvent.const_574));
