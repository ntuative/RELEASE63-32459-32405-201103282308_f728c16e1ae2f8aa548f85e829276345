package com.sulake.habbo.moderation
{
   ...
   public class RoomToolCtrl implements IDisposable, TrackedWindow
   {
      ...
      private var var_1187:IDropMenuWindow;
      
      private var var_537:ITextFieldWindow;
      
      private var var_638:Boolean = true;
      ...
      public function show() : void
      {
         ...
         this.var_33.connection.send(new GetModeratorRoomInfoMessageComposer(this.var_377));
         Logger.log("BEGINNING TO SHOW: " + this.var_377);
      }
      ...
      {
         Logger.log("GOT ROOM INFO: " + param1.flatId + ", " + this.var_377);
         if(param1.flatId != this.var_377)
         {
            Logger.log("NOT THE SAME FLAT: " + param1.flatId + ", " + this.var_377);
            return;
         }
         this._data = param1;
         this.populate();
      ...
      public function populate() : void
      {
         ...
         this.var_537 = ITextFieldWindow(this._frame.findChildByName("message_input"));
         this.var_537.procedure = this.onInputClick;
         this.var_1187 = IDropMenuWindow(this._frame.findChildByName("msgTemplatesSelect"));
         this.prepareMsgSelect(this.var_1187);
         this.var_1187.procedure = this.onSelectTemplate;
         this.var_959 = ICheckBoxWindow(this._frame.findChildByName("kick_check"));
      ...
      private function onChatlog(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         this.var_33.windowTracker.show(new ChatlogCtrl(new GetRoomChatlogMessageComposer(0,this._data.flatId),this.var_33,WindowTracker.const_891,this._data.flatId),this._frame,false,false,true);
      }
      ...
      {
         if(this.var_638 || this.var_537.text == "")
         {
            this.var_33.windowManager.alert("Alert","You must input a message to the user",0,this.onAlertClose);
            return;
         }
         var _loc2_:int = this.determineAction(param1,this.var_959.isSelected);
         this.var_33.connection.send(new ModeratorActionMessageComposer(ModeratorActionMessageComposer.const_1501,_loc2_,this.var_537.text,"",""));
         if(this.var_1457.isSelected || this.var_1458.isSelected || this.var_959.isSelected)
         {
            this.var_33.connection.send(new ModerateRoomMessageComposer(this._data.flatId,this.var_1457.isSelected,this.var_1458.isSelected,this.var_959.isSelected));
         }
         this.dispose();
      }
      ...
      private function determineAction(param1:Boolean, param2:Boolean) : int
      {
         if(param2)
         {
            return !!param1 ? 0 : int(ModeratorActionMessageComposer.const_1624);
         }
         return !!param1 ? 0 : int(ModeratorActionMessageComposer.const_1560);
      }
      ...
      private function onSelectTemplate(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         var _loc3_:String = this.var_33.initMsg.roomMessageTemplates[this.var_1187.selection];
         if(_loc3_ != null)
         {
            this.var_638 = false;
            this.var_537.text = _loc3_;
         }
      }
   }
}
